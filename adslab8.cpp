...
int main(){
    ...
    cout << "Enter the number of different heap sizes: ";
    cin >> n;

    cout << "Enter " << n << " heap sizes (e.g., 100 200 300 ...):\n";
    for(int i=0;i<n;i++){
        cout << "Size " << (i+1) << ": ";
        cin >> sizes[i];
    }

    long long binary[1000][4];
    long long leftist[1000][4];
    long long binomial[1000][4];

    for(int idx=0;idx<n;idx++){
        int heap_size = sizes[idx];

        // Binary Heap
        {
            Metrics mc,mi,md,mm;
            BinaryMinHeap H1;
            for(int i=0;i<heap_size;i++) H1.insert(gen_val(i,heap_size,7),mc);
            H1.insert(gen_val(heap_size+1,heap_size,7),mi);
            H1.extract_min(md);
            BinaryMinHeap H2;
            for(int i=0;i<heap_size;i++) H2.insert(gen_val(i+3,heap_size,7),mc);
        ...
        // Leftist Heap
        {
            Metrics mc,mi,md,mm;
            LeftistMinHeap L1;
            for(int i=0;i<heap_size;i++) L1.insert(gen_val(i,heap_size,5),mc);
            L1.insert(gen_val(heap_size+1,heap_size,5),mi);
            L1.extract_min(md);
            LeftistMinHeap L2;
            for(int i=0;i<heap_size;i++) L2.insert(gen_val(i+2,heap_size,5),mc);
        ...
        // Binomial Heap
        {
            Metrics mc,mi,md,mm;
            BinomialMinHeap B1;
            for(int i=0;i<heap_size;i++) B1.insert(gen_val(i,heap_size,3),mc);
            B1.insert(gen_val(heap_size+1,heap_size,3),mi);
            B1.extract_min(md);
            BinomialMinHeap B2;
            for(int i=0;i<heap_size;i++) B2.insert(gen_val(i+1,heap_size,3),mc);
